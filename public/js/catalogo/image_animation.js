...
// -------------------------------------------------------------------------------------------
    ...
    function activate_shortcode_scripts(container)
	{
		...
		//activates animation for Counter
		if($.fn.themeapt_sc_counter)
		{
			$('.counters_content', container).themeapt_sc_counter();
		}
    }
...
// -------------------------------------------------------------------------------------------
// counter shortcode javascript
// -------------------------------------------------------------------------------------------

$.fn.themeapt_sc_counter= function(options)
{
	return this.each(function()
	{
		var container = $(this), elements = container.find('.count-number');
		container.on('themeapt_start_animation', function()
		{
			elements.each(function(i)
			{
				var element = $(this);
				var start_no = element.attr('data-from');
				var end_no = element.attr('data-to'); 
				var dur = element.attr('data-refresh-interval') * element.attr('data-speed');
				setTimeout(function(){ 
					$({someValue: start_no}).animate({someValue: end_no}, {
						duration: dur,
						easing:'swing',  
						step: function() { 
	...
	container.on('themeapt_start_animation', function()
		{
			 ...
			 jQuery('.percentage').easyPieChart({
               ...
               jQuery('.percentage-light2').easyPieChart({
                    barColor: '#C02C31',
                    trackColor: false,
                    scaleColor: false,
                    lineCap: 'butt',
               ...
               jQuery('.percentage-light3').easyPieChart({
                    barColor: '#C02C31',
                    trackColor: '#dce3e5',
                    scaleColor: false,
                    lineCap: 'butt',
               ...
               jQuery('.percentage-light4').easyPieChart({
                    barColor: '#C02C31',
                    trackColor: '#dce3e5',
                    scaleColor: false,
                    lineCap: 'butt',
